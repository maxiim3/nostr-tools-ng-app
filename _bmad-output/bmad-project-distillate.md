...
- Current product focus remains the francophone starter pack, reliable Nostr authentication, Supabase-backed member persistence, protected admin member management, and Nostr/Lightning payment surfaces; sources: `README.md`, `src/features/packs/README.md`, `docs/auth/`, `docs/zaps/`.
...
- Layout: feature-first pseudo-DDD/hexagonal Angular app; `src/app` routes/config/root component; `src/core` cross-cutting services; `src/features` product features; `src/shared` only generic UI primitives/design tokens/pure helpers/simple shared types; source: `_bmad-output/project-context.md`.
- Feature layers: `domain` = pure business types/value objects/rules/functions with no Angular/UI/Nostr/HTTP/Supabase; `application` = use cases/orchestration/ports with no UI dependencies; `infrastructure` = Nostr/NDK/HTTP/Supabase/relay/event adapters and port implementations; `presentation` = Angular pages/components/view models/UI state/template bindings; dependency direction presentation -> application -> domain, infrastructure implements ports; source: `_bmad-output/project-context.md`.
...
- Relays centralized in `src/core/nostr/infrastructure/relay.config.ts`: `wss://relay.damus.io`, `wss://relay.nostr.band`, `wss://nostr.oxtr.dev`, `wss://nostr-pub.wellorder.net`, `wss://nos.lol`, `wss://relay.primal.net`; source: `src/core/nostr/README.md`.
...
- Effective source precedence: current implementation facts and maintained protocol docs under `docs/` > BMAD project context > generated planning artifacts. If docs conflict, preserve implementation facts and mark planning uncertainty.
- Maintained protocol documentation lives in `docs/`; use it for auth, signer, zap, LNURL, NWC, and Lightning integration context.
...
- Implemented config: francophone pack config currently includes `slug`, `adminNpubs`, `starterPackUrl`, `followUrl`, `externalLoginUrl`, `zapHref`; source: `src/features/packs/README.md`.

## Active/Pending Work

- Active/pending work should be derived from BMAD stories and current implementation state, not deleted docs. Use `docs/` only for protocol and integration context.
...
- Product limits remain planning-owned; do not infer them from protocol docs.
...
- Session restore must never authenticate from cached profile alone; invalid/expired/denied/revoked signer restores return disconnected; invalid NIP-46 payload purged; source: `docs/auth/nostr-auth-rules.md`.
...
- Permission minimization target: inventory requested permissions, map each to feature need, reduce startup scope where safe, and use just-in-time prompts; source: `docs/auth/nostr-auth-rules.md`.
...
- Verification risk areas: session restore false positives from stale local data, desktop/mobile signer differences, NDK payload drift, loading state reset on exceptional paths, mobile timeout/return-to-site variance, permission reduction breaking post-login actions; source: `docs/auth/`.

- Manual admin approve/reject is no longer the primary membership workflow; auto-admit plus admin member table replaces/demotes it; source: current implementation and BMAD planning artifacts.
- Runtime SQLite storage is retired for francophone pack membership after Supabase migration; source: current implementation and Supabase migration docs.
- Bunker one-shot permission grant work is blocked by lack of clean NDK extension point and may be superseded; source: current implementation and BMAD planning artifacts.

## Open Questions / Gaps

- Open questions should be verified against current implementation and BMAD story files before execution.
