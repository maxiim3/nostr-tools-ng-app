import { resolveAdminRequestStatus, resolveUserRequestStatus } from './request-status';
...
describe('request status helpers', () => {
  ...
  it('returns idle when a rejection is newer than the latest request', () => {
    expect(resolveUserRequestStatus({ createdAt: 20 }, { createdAt: 30, status: 'rejected' })).toBe(
      'idle'
    );
  });

  it('returns idle when request and rejection have the same timestamp', () => {
    expect(resolveUserRequestStatus({ createdAt: 30 }, { createdAt: 30, status: 'rejected' })).toBe(
      'idle'
    );
  });
  ...
  it('returns rejected for admin when timestamps are equal and decision is rejected', () => {
    expect(
      resolveAdminRequestStatus({ createdAt: 20 }, { createdAt: 20, status: 'rejected' })
    ).toBe('rejected');
  });

  it('returns rejected for admin when the latest decision rejects', () => {
    expect(
      resolveAdminRequestStatus({ createdAt: 10 }, { createdAt: 20, status: 'rejected' })
    ).toBe('rejected');
  });
});
